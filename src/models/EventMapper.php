...
class EventMapper extends ApiMapper
{
    ...
    public function getEventsHostedByUser($user_id, $resultsperpage, $start, $verbose = false)
    {
        ...
        $sql = 'select events.*, '
               ...
               . 'abs(datediff(from_unixtime(events.event_start),
                from_unixtime(' . mktime(0, 0, 0) . '))) as score, '
               . 'CASE
                WHEN (((events.event_start - 3600*24) < ' . mktime(0, 0, 0) .
               ') and (events.event_start + (3*30*3600*24)) > ' . mktime(0, 0, 0) .
               ') THEN 1 ELSE 0 END as comments_enabled ' .
               'from events ' .
    ...
    {
        $sql    = "UPDATE events e SET talk_count = (SELECT COUNT(*) FROM talks t ".
                  "WHERE t.event_id = e.ID) WHERE e.ID = :event_id;";
        $stmt   = $this->_db->prepare($sql);
    ...
    {
        $sql    = "UPDATE events e SET comment_count = (SELECT COUNT(*) FROM event_comments ec ".
                  "WHERE ec.event_id = e.ID) WHERE e.ID = :event_id;";
        $stmt   = $this->_db->prepare($sql);
        $result = $stmt->execute(array("event_id" => $event_id));
